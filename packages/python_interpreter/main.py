...
from Aptixbot.api.event import filter
from Aptixbot.api.provider import ProviderRequest
from Aptixbot.api.message_components import Image, File
...
DEFAULT_CONFIG = {
    "sandbox": {
        "image": "Aptix/Aptixbot-code-interpreter-sandbox",
        "docker_mirror": "", # cjie.eu.org
    }
}
PATH = "data/config/python_interpreter.json"

@star.register(name="Aptixbot-python-interpreter", desc="Python 代码执行器", author="Aptix", version="0.0.1")
class Main(star.Star):
    ...
    async def initialize(self):
        ...
        if not ok:
            logger.info("Docker 不可用，代码解释器将无法使用，Aptixbot-python-interpreter 将自动禁用。")
            await self.context._star_manager.turn_off_plugin("Aptixbot-python-interpreter")
    ...
    async def file_upload(self, file_path: str):
        ...
        S3_URL = "https://s3.neko.Aptix.top/Aptixbot-s3"
    ...
    async def on_llm_req(self, event: AptixMessageEvent, request: ProviderRequest):
    ...
    @pi.command("mirror")
    async def pi_mirror(self, event: AptixMessageEvent, url: str = ""):
        '''Docker 镜像地址'''
        if not url:
            yield event.plain_result(f"""当前 Docker 镜像地址: {self.config['sandbox']['docker_mirror']}。
使用 `pi mirror <url>` 来设置 Docker 镜像地址。
您所设置的 Docker 镜像地址将会自动加在 Docker 镜像名前。如: `Aptix/Aptixbot-code-interpreter-sandbox` -> `cjie.eu.org/Aptix/Aptixbot-code-interpreter-sandbox`。
""")
        else:
    ...
    @pi.command("repull")
    async def pi_repull(self, event: AptixMessageEvent):
        '''重新拉取沙箱镜像'''
        docker = aiodocker.Docker()
    ...
    async def python_interpreter(self, event: AptixMessageEvent):
        ...
        for i in range(n):
            ...
            container = await docker.containers.run({
                ...
                "HostConfig": {
                    "Binds": [
                        f"{self.shared_path}:/Aptixbot_sandbox/shared:ro",
                        f"{output_path}:/Aptixbot_sandbox/output:rw",
                        f"{workplace_path}:/Aptixbot_sandbox:rw",
                    ]
                },
                "Env": [
            ...
            # 发送结果
            pattern = r"\[AptixBOT_(TEXT|IMAGE|FILE)_OUTPUT#\w+\]: (.*)"
            ok = False
            traceback = ""
            for idx, log in enumerate(logs):
                ...
                if match:
                    ...
                    if match.group(1) == "TEXT":
                        ...
                        logger.info(f"文件上传到 AptixBot 云节点: {file_s3_url}")
                        file_name = os.path.basename(file_path)
    ...
    async def pi_cleanfile(self, event: AptixMessageEvent):
        '''清理用户上传的文件'''
