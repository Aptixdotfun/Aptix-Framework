...
@star.register(name="Aptixbot-reminder", desc="使用 LLM 待办提醒", author="Aptix", version="0.0.1")
class Main(star.Star):
    ...
    def __init__(self, context: star.Context) -> None:
        ...
        # set and load config
        if not os.path.exists("data/Aptixbot-reminder.json"):
            with open("data/Aptixbot-reminder.json", "w") as f:
                f.write("{}")
        with open("data/Aptixbot-reminder.json", "r") as f:
    ...
    @llm_tool("reminder")
    async def reminder_tool(self, event: AptixMessageEvent, text: str=None, datetime_str: str = None, cron_expression: str = None, human_readable_cron: str = None):
    ...
    @reminder.command("ls")
    async def reminder_ls(self, event: AptixMessageEvent):
    ...
    @reminder.command("rm")
    async def reminder_rm(self, event: AptixMessageEvent, index: int):
