...
from Aptixbot.core.message.message_event_result import MessageEventResult, MessageChain
from Aptixbot.core.message.components import At
...
class WakingCheckStage(Stage):
    ...
    async def initialize(self, ctx: PipelineContext) -> None:
        self.ctx = ctx
        self.no_permission_reply = self.ctx.Aptixbot_config["platform_settings"].get(
            "no_permission_reply", True
        )

    async def process(
        self, event: AptixMessageEvent
    ) -> Union[None, AsyncGenerator[None, None]]:
        # 设置 sender 身份
        event.message_str = event.message_str.strip()
        for admin_id in self.ctx.Aptixbot_config["admins_id"]:
            if event.get_sender_id() == admin_id:
                event.role = "admin"
                break

        # 检查 wake
        wake_prefixes = self.ctx.Aptixbot_config["wake_prefix"]
                ...
                try:
                    if isinstance(filter, CommandGroupFilter):
                        ...
                        ok, child_command_handler_md = filter.filter(
                            event, self.ctx.Aptixbot_config
                        )
                        if not ok:
                            passed = False
                        else:
                        ...
                        if not filter.filter(event, self.ctx.Aptixbot_config):
