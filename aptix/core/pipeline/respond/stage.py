...
class RespondStage(Stage):
    async def initialize(self, ctx: PipelineContext):
        ...
        # 分段回复
        self.enable_seg: bool = ctx.Aptixbot_config['platform_settings']['segmented_reply']['enable']
        self.only_llm_result = ctx.Aptixbot_config['platform_settings']['segmented_reply']['only_llm_result']
        interval_str: str = ctx.Aptixbot_config['platform_settings']['segmented_reply']['interval']
    ...
    async def process(self, event: AptixMessageEvent) -> Union[None, AsyncGenerator[None, None]]:
        ...
        if len(result.chain) > 0:
            ...
            logger.info(f"AptixBot -> {event.get_sender_name()}/{event.get_sender_id()}: {event._outline_chain(result.chain)}")
