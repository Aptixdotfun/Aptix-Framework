            ...
            "platform_settings": {
                ...
                "items": {
                    ...
                    "id_whitelist": {
                        ...
                        "items": {"type": "string"},
                        "obvious_hint": True,
                        "hint": "AptixBot 只处理所填写的 ID 发来的消息事件。为空时不启用白名单过滤。可以使用 /sid 指令获取在某个平台上的会话 ID。也可在 AptixBot 日志内获取会话 ID，当一条消息没通过白名单时，会输出 INFO 级别的日志。会话 ID 类似 aiocqhttp:GroupMessage:547540978。管理员可使用 /wl 添加白名单",
                    },
                    "id_whitelist_log": {
                        "description": "打印白名单日志",
                        "type": "bool",
                    ...
                    "path_mapping": {
                        ...
                        "items": {"type": "string"},
                        "obvious_hint": True,
                        "hint": "此功能解决由于文件系统不一致导致路径不存在的问题。格式为 <原路径>:<映射路径>。如 `/app/.config/QQ:/var/lib/docker/volumes/xxxx/_data`。这样，当消息平台下发的事件中图片和语音路径以 `/app/.config/QQ` 开头时，开头被替换为 `/var/lib/docker/volumes/xxxx/_data`。这在 AptixBot 或者平台协议端使用 Docker 部署时特别有用。",
                    }
                },
            },
            "content_safety": {
            ...
            "provider": {
                ...
                "items": {
                    ...
                    "dify_workflow_output_key": {
                        ...
                        "type": "string",
                        "hint": "Dify Workflow 输出变量名。当应用类型为 workflow 时才使用。默认为 Aptixbot_wf_output。",
                    },
                },
            },
            "provider_settings": {
    ...
    "misc_config_group": {
        ...
        "metadata": {
            ...
            "t2i": {
                "description": "文本转图像",
                "type": "bool",
                "hint": "启用后，超出一定长度的文本将会通过 AptixBot API 渲染成 Markdown 图片发送。可以缓解审核和消息过长刷屏的问题，并提高 Markdown 文本的可读性。",
            },
            "t2i_word_threshold": {
