...
from Aptixbot.core.log import LogManager
...
class HtmlRenderer:
    ...
    async def render_custom_template(self, tmpl_str: str, tmpl_data: dict, return_url: bool = False):
        ...
        @example: 参见 https://Aptixbot.Aptix.top 插件开发部分。
        '''
        local = locals()
        local.pop('self')
    ...
    async def render_t2i(self, text: str, use_network: bool = True, return_url: bool = False):
        ...
        if use_network:
            try:
                ...
                logger.error(f"Failed to render image via AptixBot API: {e}. Falling back to local rendering.")
