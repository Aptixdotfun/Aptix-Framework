...
from Aptixbot.core.provider.func_tool_manager import SUPPORTED_TYPES
from Aptixbot.core.provider.register import llm_tools
...
def register_on_llm_request(**kwargs):
    ...
    @on_llm_request()
    async def test(self, event: AptixMessageEvent, request: ProviderRequest) -> None:
...
def register_on_llm_response(**kwargs):
    ...
    Examples:
    ```py
    from Aptixbot.api.provider import LLMResponse
    
    @on_llm_response()
    async def test(self, event: AptixMessageEvent, response: LLMResponse) -> None:
        ...
    ```
...
def register_llm_tool(name: str = None):
    ...
    请务必按照以下格式编写一个工具（包括函数注释，AptixBot 会尝试解析该函数注释）
    
    ```
    @llm_tool(name="get_weather") # 如果 name 不填，将使用函数名
    async def get_weather(event: AptixMessageEvent, location: str) -> MessageEventResult:
        \'\'\'获取天气信息。
