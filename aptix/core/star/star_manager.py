...
from Aptixbot.core import logger, sp, pip_installer
from .context import Context
from . import StarMetadata
from .updator import PluginUpdator
from Aptixbot.core.utils.io import remove_dir
...
class PluginManager:
    ...
    async def uninstall_plugin(self, plugin_name: str):
        ...
        if not plugin:
            raise Exception("插件不存在。")
        if plugin.reserved:
            raise Exception("该插件是 AptixBot 保留插件，无法卸载。")
    ...
    async def update_plugin(self, plugin_name: str):
        ...
        if not plugin:
            raise Exception("插件不存在。")
        if plugin.reserved:
            raise Exception("该插件是 AptixBot 保留插件，无法更新。")
