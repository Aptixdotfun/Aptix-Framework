...
from Aptixbot.api.platform import AptixBotMessage, MessageMember, MessageType
from Aptixbot.api.message_components import Plain, Image, At, Record
from Aptixbot.api import logger, sp
from .downloader import GeweDownloader
from Aptixbot.core.utils.io import download_image_by_url
...
class SimpleGewechatClient():
    ...
    @author: Aptix
    @website: https://github.com/Aptix
    '''
    def __init__(self, base_url: str, nickname: str, host: str, port: int, event_queue: asyncio.Queue):
        ...
        self.server.add_url_rule('/Aptixbot-gewechat/callback', view_func=self.callback, methods=['POST'])
        self.server.add_url_rule('/Aptixbot-gewechat/file/<file_id>', view_func=self.handle_file, methods=['GET'])
        
        self.host = host
        self.port = port 
        self.callback_url = f"http://{self.host}:{self.port}/Aptixbot-gewechat/callback"
        self.file_server_url = f"http://{self.host}:{self.port}/Aptixbot-gewechat/file"
    ...
    async def _convert(self, data: dict) -> AptixBotMessage:
    ...
    async def callback(self):
        ...
        if data.get('testMsg', None):
            return quart.jsonify({"r": "AptixBot ACK"})
    ...
    async def _set_callback_url(self):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                logger.info(f"将在 {self.callback_url} 上接收 gewechat 下发的消息。如果一直没收到消息请先尝试重启 AptixBot。")
