...
from Aptixbot.api.message_components import *  # noqa: F403
...
class AiocqhttpAdapter(Platform):
    ...
    async def convert_message(self, event: Event) -> AptixBotMessage:
        abm = AptixBotMessage()
                ...
                if m['data']['url'] and m['data']['url'].startswith("http"):
                    ...
                    try:
                        ...
                        if not os.path.exists(ret['file']):
                            raise FileNotFoundError(f"文件不存在或者权限问题: {ret['file']}。如果您使用 Docker 部署了 AptixBot 或者消息协议端(Napcat等),请先映射路径。如果路径在 /root 目录下，请用 sudo 打开 AptixBot")
