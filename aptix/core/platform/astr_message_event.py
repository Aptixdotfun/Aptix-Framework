...
from .Aptixbot_message import AptixBotMessage
...
from Aptixbot.core.message.components import Plain, Image, BaseMessageComponent, Face, At, AtAll, Forward
...
class AptixMessageEvent(abc.ABC):
    def __init__(self, 
                message_str: str,
                message_obj: AptixBotMessage,
                platform_meta: PlatformMetadata,
                session_id: str,):
        self.message_str = message_str
        '''纯文本的消息'''
        self.message_obj = message_obj
        '''消息对象，AptixBotMessage。带有完整的消息结构。'''
    ...
    def set_result(self, result: Union[MessageEventResult, str]):
        ...
        Example:
        ```
        async def ban_handler(self, event: AptixMessageEvent):
        ...
        async def check_count(self, event: AptixMessageEvent):
            self.count += 1
