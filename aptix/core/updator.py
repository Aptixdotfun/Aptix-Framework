...
from Aptixbot.core.utils.io import download_file

class AptixBotUpdator(RepoZipUpdator):
    def __init__(self, repo_mirror: str = "") -> None:
        super().__init__(repo_mirror)
        self.MAIN_PATH = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), "../../"))
        self.AptixBOT_RELEASE_API = "https://api.Aptix.top/releases"
    ...
    async def check_update(self, url: str, current_version: str) -> ReleaseInfo:
        return await super().check_update(self.AptixBOT_RELEASE_API, VERSION)
        
    async def update(self, reboot = False, latest = True, version = None):
        update_data = await self.fetch_release_info(self.AptixBOT_RELEASE_API, latest)
        file_url = None
        ...
        if latest:
            ...
            file_url = "https://github.com/Aptix/AptixBot/archive/" + version + ".zip"
