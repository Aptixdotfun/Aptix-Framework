...
from Aptixbot.core.config.default import CONFIG_METADATA_2
from Aptixbot.core.platform.Aptix_message_event import AptixMessageEvent
from Aptixbot.core.platform.Aptixbot_message import AptixBotMessage, MessageMember, MessageType
from Aptixbot.core.message.message_event_result import MessageChain, ResultContentType
from Aptixbot.core.message.components import Plain, At
from Aptixbot.core.platform.platform_metadata import PlatformMetadata
...
class FakeAptixMessageEvent(AptixMessageEvent):
    def __init__(self, abm: AptixBotMessage = None):
    ...
    def create_fake_event(
        ...
        return FakeAptixMessageEvent(abm)
...
async def test_pipeline_wakeup(pipeline_scheduler: PipelineScheduler, caplog):
    ...
    # 群聊无 @ 无指令
    caplog.clear()
    mock_event = FakeAptixMessageEvent.create_fake_event("test", is_group=True)
    ...
    # 群聊有 @ 无指令
    mock_event = FakeAptixMessageEvent.create_fake_event("test", is_group=True, is_at=True)
    with caplog.at_level(logging.DEBUG):
        await pipeline_scheduler.execute(mock_event)
    assert any("执行阶段 WhitelistCheckStage" in message for message in caplog.messages)
    # 群聊有指令
    mock_event = FakeAptixMessageEvent.create_fake_event("/help", is_group=True, session_id=SESSION_ID_IN_WHITELIST)
    await pipeline_scheduler.execute(mock_event)
    assert mock_event._has_send_oper is True

@pytest.mark.asyncio
async def test_pipeline_wl(pipeline_scheduler: PipelineScheduler, config: AptixBotConfig, caplog):
    caplog.clear()
    mock_event = FakeAptixMessageEvent.create_fake_event("test", SESSION_ID_IN_WHITELIST, sender_id="123")
    with caplog.at_level(logging.INFO):
        await pipeline_scheduler.execute(mock_event)
    assert any("不在会话白名单中，已终止事件传播。" not in message for message in caplog.messages), "日志中未找到预期的消息"

    mock_event = FakeAptixMessageEvent.create_fake_event("test", sender_id="123")
    with caplog.at_level(logging.INFO):
        await pipeline_scheduler.execute(mock_event)
...
async def test_pipeline_content_safety(pipeline_scheduler: PipelineScheduler, caplog):
    # 测试默认屏蔽词
    caplog.clear()
    mock_event = FakeAptixMessageEvent.create_fake_event("色情", session_id=SESSION_ID_IN_WHITELIST) # 测试需要。
    with caplog.at_level(logging.INFO):
        await pipeline_scheduler.execute(mock_event)
    assert any("内容安全检查不通过" in message for message in caplog.messages), "日志中未找到预期的消息"
    # 测试额外屏蔽词
    mock_event = FakeAptixMessageEvent.create_fake_event("TEST_NEGATIVE", session_id=SESSION_ID_IN_WHITELIST)
    with caplog.at_level(logging.INFO):
        await pipeline_scheduler.execute(mock_event)
    assert any("内容安全检查不通过" in message for message in caplog.messages), "日志中未找到预期的消息"
    mock_event = FakeAptixMessageEvent.create_fake_event("_TEST_NEGATIVE", session_id=SESSION_ID_IN_WHITELIST)
    with caplog.at_level(logging.INFO):
        await pipeline_scheduler.execute(mock_event)
...
async def test_pipeline_llm(pipeline_scheduler: PipelineScheduler, caplog):
    caplog.clear()
    mock_event = FakeAptixMessageEvent.create_fake_event("just reply me `OK`", session_id=SESSION_ID_IN_WHITELIST)
    with caplog.at_level(logging.DEBUG):
        await pipeline_scheduler.execute(mock_event)
...
async def test_pipeline_websearch(pipeline_scheduler: PipelineScheduler, caplog):
    caplog.clear()
    mock_event = FakeAptixMessageEvent.create_fake_event("help me search the latest OpenAI news", session_id=SESSION_ID_IN_WHITELIST)
    with caplog.at_level(logging.DEBUG):
        await pipeline_scheduler.execute(mock_event)
    assert any("请求 LLM" in message for message in caplog.messages)
...
async def test_commands(pipeline_scheduler: PipelineScheduler, caplog):
    for command in TEST_COMMANDS:
        caplog.clear()
        mock_event = FakeAptixMessageEvent.create_fake_event(command[0], session_id=SESSION_ID_IN_WHITELIST)
        with caplog.at_level(logging.DEBUG):
            await pipeline_scheduler.execute(mock_event)
